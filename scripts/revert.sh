...
for file in $(find ../src/main/java -type f -name "*.java"); do
	sed -i \
		...
		\
		-e 's/@NotNull/@Nonnull/g' \
		-e 's/import org\.jetbrains\.annotations\.Nullable;/import javax\.annotation\.Nullable;/g' \
		-e 's/import org\.jetbrains\.annotations\.NotNull;/import javax\.annotation\.Nonnull;/g' \
	$file
